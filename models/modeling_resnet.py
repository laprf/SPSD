...
def conv3x3(cin, cout, stride=1, groups=1, bias=False):
    '''3*3 conv with standard weight'''
    return StdConv2d(cin, cout, kernel_size=3, stride=stride,
                     padding=1, bias=bias, groups=groups)


def conv1x1(cin, cout, stride=1, bias=False):
    '''1*1 conv with standard weight'''
...
class ResNetV2(nn.Module):
    ...
    def forward(self, x):
        res_feat = []
        x = self.root(x)
        for name, block in enumerate(self.body):
            x = block(x)
            res_feat.append(x)  # resnet 50, stage 2,3,5
        return (res_feat[0], res_feat[1]), res_feat[2]
